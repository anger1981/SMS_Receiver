...
uses
  Windows, SysUtils, SyncObjs, DateUtils, Dialogs;

const
  MAX_COUNT_SYM_LATIN = 170;
  CRLF = #$D#$A;
  BUFF_SIZE = $FF;
...
type
  ...
  public
    ...
    function GetSMS_Int(AIndex: Integer): TSMSMessage;
    function GetAllSMS: TSMSMessages;
    function GetAllSMS_Int: TSMSMessages;
    function DeleteSMS(AIndex: Integer): Boolean;
    function DeleteSMSes(LSMSs: TSMSMessages): Boolean;
  end;
...
begin
  ...
  begin
    ...
    begin
      Result.ID_Message := StrToInt(LHeader[0]);
      Result.Number := LHeader[2];
      Result.Time := LHeader[4] + ',' + LHeader[5];
      if (StrIsHex(LMess[1])) then Result.Text := UCSToAnsi(LMess[1])
                              else Result.Text := LMess[1]; // UCSToAnsi(LMess[1]);
    end;
  end;
end;
...
function TGsmSms.GetSMS_Int(AIndex: Integer): TSMSMessage;
begin
  Open;
  try
    SetModemMode(mdInt);
...
function TGsmSms.DeleteSMSes(LSMSs: TSMSMessages): Boolean;
var
  i: Integer;
begin
  Open;
  ...
  try
    ...
    Result := Result and DeleteSMSMessage(LSMSs[i].ID_Message);
  finally
    Close;
  end;
end;
...
function TGsmSms.GetAllSMS_Int: TSMSMessages;
begin
  Open;
  try
    SetModemMode(mdInt);
